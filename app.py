...
# ... (El código anterior de NOTAS termina aquí) ...
                    ...
                    with p_mid:
                        ...
                        # Definimos el tamaño del plano (ancho, alto en pulgadas)
                        fig, ax = plt.subplots(figsize=(16, 9))
                        
                        # --- CONFIGURACIÓN DEL PLANO (La "cuadrícula" de la bodega) ---
                        # Definimos los límites del plano (0,0 es la esquina inferior izquierda)
                        st_bodega_x = 100
                        st_bodega_y = 60
                        ax.set_xlim(0, st_bodega_x)
                        ax.set_ylim(0, st_bodega_y)
                        
                        # Definimos las secciones clave de la bodega
                        secciones_bodega = {
                            'H75': {'x': 0, 'y': 50, 'w': 100, 'h': 10, 'color': '#d4edda'}, # Verde claro ESTRA
                            'H76': {'x': 0, 'y': 0, 'w': 100, 'h': 10, 'color': '#d4edda'}, # Verde claro ESTRA
                            # ZONA CENTRAL DE ESTIBAS (Simplificada)
                            'ESTIBAS 1-4': {'x': 0, 'y': 10, 'w': 20, 'h': 40, 'color': '#e2e3e5'}, # Gris claro
                            'ESTIBAS 7-10': {'x': 20, 'y': 10, 'w': 20, 'h': 40, 'color': '#e2e3e5'},
                            'ESTIBAS 13-16': {'x': 40, 'y': 10, 'w': 20, 'h': 40, 'color': '#e2e3e5'},
                            'ESTIBAS 18-21': {'x': 60, 'y': 10, 'w': 20, 'h': 40, 'color': '#e2e3e5'},
                            'ESTIBAS 23-26': {'x': 80, 'y': 10, 'w': 20, 'h': 40, 'color': '#e2e3e5'}
                        }
                        
                        # Dibujamos las secciones de fondo del plano
                        for nombre, config in secciones_bodega.items():
                            rect = plt.Rectangle((config['x'], config['y']), config['w'], config['h'], facecolor=config['color'], edgecolor='white', linewidth=2)
                            ax.add_patch(rect)
                            # Añadimos el texto de la sección centrado
                            ax.text(config['x'] + (config['w'] / 2), config['y'] + (config['h'] / 2), nombre, ha='center', va='center', fontsize=20, weight='bold', color='#155724')
                        ...
                        estibas_coord = {
                            ...
                            # Estibas inferiores (H76) - Izquierda a derecha
                            '6': {'x': 10, 'y': 17}, '12': {'x': 30, 'y': 17}, '17': {'x': 50, 'y': 17}, '25': {'x': 70, 'y': 17},
                            # Zonas centrales (Aproximación por "cuadras" del plano original)
                            '1': {'x': 5, 'y': 35}, '2': {'x': 5, 'y': 30}, '3': {'x': 5, 'y': 25}, '4': {'x': 5, 'y': 20},
                            '7': {'x': 25, 'y': 35}, '8': {'x': 25, 'y': 30}, '9': {'x': 25, 'y': 25}, '10': {'x': 25, 'y': 20},
                            '13': {'x': 45, 'y': 35}, '14': {'x': 45, 'y': 30}, '15': {'x': 45, 'y': 25}, '16': {'x': 45, 'y': 20},
                            '18': {'x': 65, 'y': 35}, '19': {'x': 65, 'y': 30}, '20': {'x': 65, 'y': 25}, '21': {'x': 65, 'y': 20},
                            '23': {'x': 85, 'y': 35}, '24': {'x': 85, 'y': 30}
                        }
                        
                        # Dibujamos todas las estibas en el plano como bloques grises
                        for num_estiba, coord in estibas_coord.items():
                            rect_estiba = plt.Rectangle((coord['x'], coord['y']), 10, 4, facecolor='#f8f9fa', edgecolor='#6c757d', linewidth=1)
                            ax.add_patch(rect_estiba)
                            # Texto del número de estiba
                        ...
                        match = re.search(r'ESTIBA (\d+)', texto_nota)
                        
                        if match:
                            num_estiba_encontrada = match.group(1) # Saca solo el número (ej: '5')
                            
                            # Verificamos que tengamos coordenadas para esta estiba
                            if num_estiba_encontrada in estibas_coord:
                                coord_e = estibas_coord[num_estiba_encontrada]
                                
                                # 1. Resaltamos el bloque en ROJO ESTRA (IGUAL QUE TU PLANO)
                                rect_resaltado = plt.Rectangle((coord_e['x'], coord_e['y']), 10, 4, facecolor='#f8d7da', edgecolor='#721c24', linewidth=2)
                                ax.add_patch(rect_resaltado)
                                
                                # 2. Texto grande y ROJO en el centro del bloque
                                ax.text(coord_e['x'] + 5, coord_e['y'] + 2, f"AQUÍ ESTÁ", ha='center', va='center', fontsize=14, weight='bold', color='#721c24')
                                ax.text(coord_e['x'] + 5, coord_e['y'] + 0.5, f"EST {num_estiba_encontrada}", ha='center', va='center', fontsize=10, weight='normal', color='#721c24')
                                
                                # 3. Flecha grande de navegación profesional
                                ax.annotate(f'UBICACIÓN MOLDE: {codigo_limpio_input}', 
                                            xy=(coord_e['x'] + 5, coord_e['y'] + 4),  # Punta de la flecha
                                            xytext=(coord_e['x'] + 5, coord_e['y'] + 10), # Posición del texto
                                            arrowprops=dict(facecolor='#28a745', shrink=0.05, width=10, headwidth=25), # Flecha VERDE ESTRA
                                            fontsize=16, weight='bold', ha='center', color='#155724')
                        
                        # --- LIMPIEZA FINAL DEL PLANO ---
                        ax.set_aspect('equal') # Para que no se deformen los rectángulos
                        ax.axis('off') # Ocultamos los ejes numéricos del gráfico

                        # 2. Mostramos el gráfico en Streamlit profesionalmente
                        st.pyplot(fig)
                        
                        # CSS para quitar el borde feo que a veces pone Matplotlib en Streamlit
                        st.markdown("""
                            <style>
